...
def generate_insights(metrics_text, client):
    if not client: return '⚠️ OpenAI API ключ не настроен. Добавьте OPENAI_API_KEY в Secrets.'
    try:
    ...
    except Exception as e: return f'Ошибка AI: {str(e)}'
...
if uploaded_file is not None:
    ...
    if date_col and date_col != 'date': df = df.rename(columns={date_col: 'date'})
    user_col = next((c for c in df.columns if any(x in c.lower() for x in ['user', 'customer', 'client'])), None)
    if user_col and user_col != 'user_id': df = df.rename(columns={user_col: 'user_id'})
    revenue_col = next((c for c in df.columns if any(x in c.lower() for x in ['revenue', 'amount', 'sum', 'price', 'value'])), None)
    if revenue_col and revenue_col != 'revenue': df = df.rename(columns={revenue_col: 'revenue'})
    ...
    with tab1:
        ...
        else: st.info("Добавьте колонку 'date'")
    ...
    with tab2:
        ...
        else: st.info("Добавьте колонку 'event_type'")
    ...
    with tab3:
        if metrics['retention'] is not None:
            ...
            st.plotly_chart(fig2, use_container_width=True)
        else: st.info("Добавьте 'date' и 'user_id'")
